...
(function (root, factory) {
	...
	.constant('gridsterConfig', {
	    ...
	    outerMargin: true,
	    resizeOnMove: false,//gridster items will be resize if they overlap another item on move
	...
	.factory('GridsterDraggable', ['$document', '$window', 'GridsterTouch',
		function ($document, $window, GridsterTouch) {
		    function GridsterDraggable($el, scope, gridster, item, itemOptions) {
		        ...
		        var dragStartSizeX, dragStartSizeY = null;
		        ...
		        function tryFitSize(row, col) {
		            ...
		            if (item.sizeX < minSizeX || item.sizeY < minSizeY || gridster.getItems(row, col, item.sizeX, item.sizeY, item).length > 0) {
		                ...
		                if (col + item.sizeX <= gridster.columns && row + item.sizeY <= gridster.rows && gridster.getItems(row, col, item.sizeX, item.sizeY, item).length === 0) {
		                    gridster.putItem(item, row, col);
		                } else {
		                    row = item.oldRow;
		                    col = item.oldColumn;
